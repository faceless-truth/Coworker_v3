...
Data state on the production DB (2026-05-26): one row in `firms` (`mc-s-accountants`, slug = `mc-s-accountants`, `shadow_mode = true`, `is_sandbox = false`, created 2026-05-01) with `azure_tenant_id` and `azure_client_id` populated as plaintext GUIDs and the client secret encrypted into `azure_client_secret_ciphertext` (132 envelope bytes, decrypt-probe-verified against the live `MASTER_ENCRYPTION_KEY`). One row in `users`: Elio (`elio@mcands.com.au`, `role = owner`, real `azure_object_id` from Entra), created 2026-05-26 by the first end-to-end Microsoft sign-in (Task 002). Two rows in `audit_log`, hash-chained: `auth.callback.success` from the sign-in, then `user.role_changed` from `accountant` to `owner` (system-actor entry, payload records the rationale). All other tenant tables are still empty. The OAuth flow is empirically verified end-to-end; the next staff sign-ins will auto-create their `users` rows with `role = accountant` (the OAuth callback's default).
...
- **Firm bootstrap procedure is documented; first tenant is live.** The `mc-s-accountants` bootstrap completed 2026-05-26 (Task 002): Azure creds populated via `coworker bootstrap-firm`, decrypt probe round-tripped against the live master key, Elio signed in incognito and was promoted from the default `accountant` role to `owner`. The OAuth callback assigns `role = accountant` to every new user, so the firm's first user (the administrator) always needs a post-sign-in owner promotion. Use the `append_audit`-aware Python one-shot in the runbook, not raw SQL, so the audit hash chain stays intact. Full procedure: `docs/runbooks/firm-bootstrap.md`.
